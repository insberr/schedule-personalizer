...
type rollupContext = {
    ...
    load: (dt: {id: string, resolveDependencies?: boolean, assertions?: {[key: string]: string} | null, meta?: {[plugin: string]: any} | null, moduleSideEffects?: boolean | "no-treeshake" | null, syntheticNamedExports?: boolean | string | null}) => Promise<ModuleInfo>
        ...
        async transform(this: rollupContext, code: string, id: string) {
            ...
            await recurseInto(s.parsed, async (s) => {
                if (s.statement == "import") {
                    const toimp = s.args[0].data as string
                    //console.log(`Resolving ${toimp}`)
                    const resolvez = await this.resolve(toimp, id)
                    //console.log(`Resolved ${resolvez}`)
                    const loaded = await this.load({...resolvez, meta: {scs: {noEmit:true}}})
                    //console.log(`${toimp} -> ${loaded.meta.scs.source}`)
                    resolved[toimp] = loaded.meta.scs.source;
                }
            })
