...
export function DeCOH(date: Date, sCFG: SchoolScheduleConfig, schCFG: MasterSettingsSchool, water: Terms): HydratedEvent {
    // first, we determine
    const event = Determine(date, sCFG);
    console.log('Determine', event);
    // then, we collapse
    const collapsed = Collapse(event, date, sCFG, schCFG);
    console.log('Collapse', collapsed);
    // then, we override
    const overriden = Overrides(collapsed, date);
    console.log('Override', overriden);

    // finally, we hydrate
    const hydrated = Hydrate(overriden, date, water, schCFG);
...
function Collapse(event: SEvent, date: Date, schConfig: SchoolScheduleConfig, schoolConfig: MasterSettingsSchool): CollapsedEvent {
    let schedules = schConfig.schedules;
    let newEvent = produce(event, (evt) => {
        evt.message = collapseMatcher(evt.message, schoolConfig, date) + (evt.message ? '<br>' : '');
        evt.overrides.enabled = collapseMatcher(evt.overrides.enabled, schoolConfig, date);
        for (let key in evt.overrides.settings) {
            evt.overrides.settings[key] = collapseMatcher(evt.overrides.settings[key], schoolConfig, date);
        }
    ...
    if (!collapsed.schedule) throw new Error('Schedule not found: ' + newEvent.schedule);
    return collapsed;
}
...
function Overrides(event: CollapsedEvent, date: Date) {
    ...
    enabledOverrides.forEach((override) => {
        ...
        if (func) {
            try {
                console.log('Executing overide: ' + override, evt, settings[override]);
                evt = produce(evt, (d: CollapsedEvent) => func(d, settings[override], date));
                console.log('->', evt);
            } catch (err) {
                console.error(err);
            }
...
function Hydrate(evt: CollapsedEvent, date: Date, water: Terms, School: MasterSettingsSchool): HydratedEvent {
    ...
    let periods: DisplayCL[] = evt.schedule.periods.map((period) => {
        if (period.id == ClassIDS.Period || period.id == ClassIDS.Advisory) {
            // merge with class
            let classFor = classes.find((cls) => cls.classID == period.id && (cls.classID == ClassIDS.Advisory || period.num == cls.period));
            if (!classFor) {
            ...
            return {
                ...
                extra: period.extra,
                name: classFor.name.trim() == '' ? 'Period ' + period.num.toString() : classFor.name,
            } as DisplayCL;
        } else {
            return {
                start: period.start,
...
export function isSchoolDay(date: Date, sCFG: SchoolScheduleConfig, schCFG: MasterSettingsSchool) {
    let event = Determine(date, sCFG);
    let collapsed = Collapse(event, date, sCFG, schCFG);
    return collapsed.schedule.periods.find((i) => ![ClassIDS.Weekend, ClassIDS.NoSchool].includes(i.id)) != undefined ? true : false;
}

export function schoolDateCount(startDate: Date, endDate: Date, sCFG: SchoolScheduleConfig, schCFG: MasterSettingsSchool) {
    let count = 0;
