...
export function DateEditor(props: Props) {
    ...
    return (<div>
        <h2> Date Editor </h2>
        <Calendar className={''} value={(props.date as DateRange)?.start ? [(props.date as DateRange)?.start, (props.date as DateRange)?.end] : props.date as Date} selectRange={true} onChange={(date: Date[]) => {
