...
function Schedule(props: ScheduleProps) {
    ...
    useEffect(() => {
        if (navigator.onLine && studentvue.isLoggedIn === true && studentvue.gotSchedules === false) {
            setCustomToast({
                ...
                body: 'We were unable to get your classes from StudentVue. StudentVue may be down. Try refreshing Schedule Personalizer.',
            });
    ...
    return (
        <div>
                ...
                <Toast onClose={() => setShowImageToast(false)} show={showImageToast} delay={10000} autohide>
                    ...
                    <Toast.Body>
                        {imageCopiedToClipboard ? (
                            ...
                            <>
                                <a href={image || '/'} rel="noreferrer" target="_parent" download={'screenshot.png'}>
                                    Download
                                </a>{' '}
                                or copy the image to save it.
                            </>
                        )}
                        <img src={image || '/'} className="rounded me-2" style={{ width: '100%', height: 'auto' }} alt="" />
                    </Toast.Body>
                </Toast>
            </ToastContainer>
            <SchHeader
            ...
            <Slide>
                <Center>
                    <div ref={screenref} style={{ padding: '3em', fontFamily: 'Roboto' }}>
                            ...
                            {props.sch.map((period, i) => {
                                return (
                                    <Row
                                        className="crow"
                                        key={
                                            period.classID.toString() +
                                            period.period?.toString() +
                                            period.name +
                                            `${period.startTime.hours}${period.startTime.minutes}${period.startTime.seconds}`
                                        }
                                    >
                                        <ScheduleEntry
                                            key={i.toString()}
                        ...
                        <div>
                            {props.event.isEvent ? (
                                ...
                                <div style={{ marginTop: '1em' }} dangerouslySetInnerHTML={{ __html: props.event.info.message }}></div>
                            ) : null}
                        </div>
                        <br />
                        <div>
                                ...
                                <div>
                                    <div
                                        ...
                                        dangerouslySetInnerHTML={{
                                            __html: props.event.info.error || 'There was an error',
                                        }}
                                    ></div>
                                </div>
                            ) : null}
