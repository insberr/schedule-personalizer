<script type="ts">
  ...
  import { getAllSchedules, getStudentInfo, isError, type StudentVueAPIDataUserDate } from "$lib/studentvue"
  import { tweened } from 'svelte/motion';
	import { cubicOut } from 'svelte/easing';
  import { update } from "svelte-french-toast/core/store";
  import { goto } from "$app/navigation";
  import toast from "svelte-french-toast";
  import { isStudentvue, studentInfo, studentVueCreds, studentVueSchedule } from "$lib/store";
      ...
      {:else if current == LoginStates.LoggedIn}
        <div in:fade> 
          ...
          <ul class="steps steps-vertical lg:steps-horizontal">
            <li class="step" class:step-primary={$progress >= 0.0}>Obtain student info</li>
            <li class="step" class:step-primary={$progress >= 0.25}>Obtain Schedule Information</li>
            <li class="step" class:step-primary={$progress >= 0.50}>Save Credentials for refresh</li>
            <li class="step" class:step-primary={$progress >= 0.75}>Finishing up</li>
          </ul>
