<script type="ts">
    ...
    $: missingName =    terms.map((i) => i.classes.filter((c) => c.name.trim()         == "").length)
    $: missingTeacher = terms.map((i) => i.classes.filter((c) => c.teacher.name.trim() == "").length)
    $: missingRoom =    terms.map((i) => i.classes.filter((c) => c.room.trim()         == "").length)
    $: totalWarn = terms.map((_,i) => missingName[i] + missingTeacher[i] + missingRoom[i])
    $: shouldShowError = totalWarn.reduce((a,b) => a+b,0) > 0
    function finishSetup() {
        if (shouldShowError) {
            if (!confirm("There are still warnings left, are you sure you want to complete setup? (todo: replace this with a modal)")) return
        }
...
<div class="m-auto w-fit text-center">
    <div class="paper m-auto w-fit">
        ...
        <div class="w-full flex justify-between text-xs px-2">
            {#each terms as _, tid}
                <span class:text-warning={totalWarn[tid] > 0} class:text-success={totalWarn[tid] == 0}>{tid + 1}</span>
            {/each}
        </div>
    </div>
</div>
    ...
    <div class="alert alert-warning my-4 shadow-lg w-fit">
        <div class="grid gap-4 grid-rows-1 grid-cols-auto">
          ...
          <div class="grid gap-1 grid-cols-1">
            ...
            {#if missingName[term]    > 0}<span>&nbsp;&nbsp;Classes missing a name:    {missingName[term]}</span>{/if}
            {#if missingRoom[term]    > 0}<span>&nbsp;&nbsp;Classes missing a room:    {missingRoom[term]}</span>{/if}
            {#if missingTeacher[term] > 0}<span>&nbsp;&nbsp;Classes missing a teacher: {missingTeacher[term]}</span>{/if}
        </div>
          {/if}
          {/each}
        </div>
...
<button on:click={finishSetup} class="w-fit m-auto btn" class:btn-warning={shouldShowError} class:btn-success={!shouldShowError}>Finish</button>
</div>
